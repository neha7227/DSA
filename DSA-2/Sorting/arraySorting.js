...
const selectionSort = (arr, n) => {
  // Selection of minimum swap to front(left)
  //1.loop from i= 0 to n-2
  for (let i = 0; i <= n - 2; i++) {
    // n-2 steps needed to sort the array as last element will be already sorted till loop completes
...
const bubbleSort = (arr, n) => {
  ...
  for (let i = n - 1; i >= 0; i--) {
    ...
    for (let j = 0; j <= i - 1; j++) {
      // j<=i-1 as we need last element(j+1) to compare with j
      if (arr[j] > arr[j + 1]) {
    ...
    if (didSwap === 0) {
      // Opitimization to break out of swapping loop if array is laready sorted. reduces TC to O(N)
      break;
    }
    console.log("runs");
  }
...
const InsertionSort = (arr, n) => {
  for (let i = 0; i <= n - 1; i++) {
    ...
    while (j > 0 && arr[j - 1] > arr[j]) {
      //element at left of jth is greater, then swap and move left(j--) until j=0
      swap(arr, j, j - 1);
      j--;
    }
  }
